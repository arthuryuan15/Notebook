...
Recusion and dp are both spliting the original problem into multiple sub-problems and then solving them, the most essential differece between then is that dynamic programming preserves the solutions of sub-problems and avoids double calculations.
...
There are n steps stairs, Each time you can either climb one or two steps. how many distinct ways you climb to the top.

We can define a array dp to save the method of climbing stairs, dp[i] means the number of climbing the i st steps.
...
`dp[i]`： 抢到第i个房子的时候，获取的利润，属性：最大值

关注i的上一状态，根据贪心，上一个状态要不是从前两个房子过来，要不就是从前一个房子过来。
...
class Solution {
    public int rob(int[] nums) {
        ...
        return Math.max(helper(1, n-1, nums), helper(2, n, nums));
    }
    
    public int helper(int l, int r, int[] nums){
        
        int[] dp = new int[nums.length+1]; // dp[i] 走到第i个屋子获取的money
        // select frist one        
        dp[l] = nums[l-1];
        for(int i = l + 1; i <= r; i++){
...
class Solution {
    ...
    Map<TreeNode, Integer> dp0 = new HashMap<>(); // 不选当前这个点的dp
    Map<TreeNode, Integer> dp1 = new HashMap<>(); // 选当前这个点的dp
    ...
    void dfs(TreeNode root){ // 从根节点开始遍历
        ...
        // 不选当前点
        dp0.put(root, Math.max(dp0.getOrDefault(root.left, 0), dp1.getOrDefault(root.left, 0)) + Math.max(dp0.getOrDefault(root.right, 0), dp1.getOrDefault(root.right, 0)));
        
        // 选择当前点
        dp1.put(root, root.val + dp0.getOrDefault(root.left, 0) + dp0.getOrDefault(root.right, 0));
...
dp[i] [j]: 从(0,0) 到 (i,j) 的路径和。属性：最小值

关注(i,j) 上一个状态，从左边，或者上面过来
...
`dp[i][j]`从(0,0)到(i,j)的路径数，属性：最大值

关注(i,j) 前的最后一个状态`dp[i-1][j] 或 dp[i][j-1]`
...
dp[i]: 将 i 拆分成至少两个整数的和之后，这些正整数构成的积。属性：积的最大值
...
## 12. Maximum Earnings From Taxi

[leetcode](https://leetcode.com/contest/biweekly-contest-61/problems/maximum-earnings-from-taxi/)

1. Sort by end time
2. dp[i] :  Hte maximun eraning when time is i.
3. traversal by time
4. traversal all the rides
...
class Solution {
    public long maxTaxiEarnings(int n, int[][] rides) {
        ...
        for(int i = 1, j = 1; i <= n; i++){
            // ride at i time get maximum earning
            dp[i] = dp[i - 1];
            while(j <= rides.length && rides[j - 1][1] <= i){ 
                System.out.println("j is " + j);
                int start = rides[j -1][0]; // 2
                int end = rides[j - 1][1]; // 5
                int tip = rides[j - 1][2]; // 4
                System.out.println(end - start + tip);
                dp[i] = Math.max(dp[i], dp[start] + end - start + tip);
                System.out.println("dp is " + dp[i]);
                 j++;
            }
        }
...
class Solution {
    public int countOrders(int n) {
        double res = 1;
        for(int i = 1; i <= n; i++){
           res = res * i * (2 * i - 1) % (Math.pow(10, 9) + 7);
        }
        return (int)res;
    }
}
